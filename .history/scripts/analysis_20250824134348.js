...
document.addEventListener('DOMContentLoaded', function() {
    ...
    function checkApiKey() {
        ...
        if (!apiKey) {
            document.getElementById('api-key-config').classList.remove('hidden');
            return false;
        }
        document.getElementById('api-key-config').classList.add('hidden');
        return true;
    }
    ...
    // Enhanced image analysis with Gemini AI
    async function simulateImageAnalysis() {
        ...
        try {
            ...
            const analysisResult = await window.geminiAIService.getDetailedFloodAnalysis(
                imageElement.src, 
                region, 
                season
            );
